...
export async function listDevices(req: ListDevicesRequest): Promise<ListDevicesResponse> {
  ...
  try {
    ...
    return {
      ...
      hasNextPage:
        typeof r.hasNextPage === "boolean" ? r.hasNextPage : devices.length === req.pageSize,
    };
  } catch {
    return {
      ok: false,
...
async function mockListDevices(req: ListDevicesRequest): Promise<ListDevicesResponse> {
  ...
  if (!req.token.trim() || req.token === "error:401") {
    return {
      ok: false,
      status: 401,
  ...
  if (req.origin === "shared") {
    list = list.filter((device) => device.shared);
  } else if (req.origin === "linked") {
    list = list.filter((device) => !device.shared);
  }
  ...
  return {
    ...
    page: req.page,
    pageSize: req.pageSize,
    hasNextPage: start + req.pageSize < total,
  };
}
